...
namespace Payroll.UI.Business.Generate
{
  ...
  /// </summary>
  public class TcSalarySlipsCreator <T> where T : TcBusinessAnalyzedRow
  {
    ...
    public TcEmployerMetaData Data { get; set; }

    private static int middleRowHeight = 12;
    private static int middleFontSize = 9;

    public TcSalarySlipsCreator(TcYearMonth workingYearMonth, TcEmployerMetaData data, string code)
    {
        this.Data           = data;
    ...
    public void CreateAndSave(string path, string logoPath, TcBindingList<T> list)
    {
        File.Delete(path);

        Document document = CreateDocument(list, logoPath);
    ...
    private Document CreateDocument(TcBindingList<T> list, string logoPath)
    {
      ...
      this.document.Info.Author     = Data.CompanyName;

      DefineStyles();

      for (int i = 0; i < list.Count; i++)
      {
          CreateSalarySlip(i + 1, list[i], logoPath);
      }

      return this.document;
    }

    private void CreateSalarySlip(int pageNumber, T data, string logoPath)
    {
        CreatePage(pageNumber, data, logoPath);
    ...
    void CreatePage(int pageNumber, T data, string logoPath)
    {
      ...
      paragraph.AddText(string.Format("SALARY SLIP FOR\t: {0} {1}", 
          WorkingYearMonth.ToDate().ToString("MMM").ToUpper(), WorkingYearMonth.ToDate().ToString("yyyy")));
    ...
    private void AddCompanyAddress(Section section)
    {
       ...
       // paragraph.Format.SpaceBefore = "1cm";
        paragraph.AddText(Data.CompanyName);
        ...
        paragraph.AddText(Data.Telephone);
        paragraph.AddLineBreak();
        paragraph.AddText(string.Format("Fax: {0}", Data.Fax));
        SetSmallFontToParagraph(paragraph);
    ...
    private void AddAddress(Section section, T data)
    {
        ...
        paragraph.AddText(data.NameWithInitials);
        paragraph.AddLineBreak();
        paragraph.AddLineBreak();
        SetNormalFontToParagraph(paragraph);
        paragraph.Format.Alignment = ParagraphAlignment.Left;
    ...
    {
        Row row = this.table.AddRow();
        row.Format.Font.Size = middleFontSize;
