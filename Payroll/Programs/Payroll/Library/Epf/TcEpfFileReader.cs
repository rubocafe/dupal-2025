...
namespace Payroll.Library.Epf
{
    public class TcEpfFileReader
    {
        ...
        private TcEpfRow GetEpfRow(int lineNumber, string line)
        {
            ...
            data.SubmissionId                   = int.Parse(line.Substring(142, 2));                        // 2 numeric, 01 = all staff in one file / 02 = Staff in separate categories as "Executive", "non-Executive"
            data.DaysOfWork                     = TcDecimal.GetDecimalFromText(line.Substring(144, 4));     // 4.2 Numeric
            data.OCGrade                        = line.Substring(148, 3);                                   // 3 Numeric
