...
namespace Payroll.Library.Epf
{
    public class TcEpfCsvFileReader
    {
        ...
        public void ReplaceOriginData(TcEpfEmployerData origin)
        {
            foreach (TcEpfRow row in File.Rows)
            {
                ...
                row.SubmissionId         = origin.SubmissionId;
            }
        }
        ...
        private TcEpfRow GetEpfRow(TcCsvDataRow row)
        {
            ...
            data.SubmissionId            = int.Parse(row.Fields[12].Value);                      // 2 numeric, 01 = all staff in one file / 02 = Staff in separate categories as "Executive", "non-Executive"
            data.DaysOfWork             = TcDecimal.GetDecimalFromText(row.Fields[13].Value);   // 4.2 Numeric
            data.OCGrade = row.Fields[14].Value;                          // 3 Numeric
