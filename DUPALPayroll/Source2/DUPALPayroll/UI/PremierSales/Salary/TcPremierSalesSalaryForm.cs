...
{
    public partial class TcPremierSalesSalaryForm : Form
    {
        private TcPremierSalesForm master;
        private string filePath = string.Empty;

        private TcBindingList<TcPremierSalesSalaryRow> all = new TcBindingList<TcPremierSalesSalaryRow>();
        ...
        public TcPremierSalesSalaryTable SalaryTable { get; private set; }

        public TcPremierSalesSalaryForm(TcPremierSalesForm master, string filePath)
        {
            InitializeComponent();

            this.filePath           = filePath;
            this.master             = master;
            SalaryTable             = new TcPremierSalesSalaryTable();
            DataLoaded              = false;

            dataGridView.AutoGenerateColumns            = false;
            duplicatesDataGridView.AutoGenerateColumns  = false;

            source.DataSource           = new TcBindingList<TcPremierSalesSalaryRow>();
            duplicatesSource.DataSource = new TcBindingList<TcPremierSalesSalaryRow>();
            ...
            filterComboBox.Items.Add(TcEnum.GetTextForEnum<TePremierSalesSalaryFilter>(TePremierSalesSalaryFilter.All));
        ...
        private void ApplyTheme()
        {
            ...
            TcTheme.FormatGridCurrencyDisplayColumn(commissionAdvanceColumn);
            ...
            TcTheme.FormatGridCurrencyDisplayColumn(dCommissionAdvanceColumn);
        ...
        public void ReloadData()
        {
            if (File.Exists(filePath))
            {
                ...
                TcPremierSalesSalaryLoader loader = new TcPremierSalesSalaryLoader(master.SettingsForm.WorkingYearMonth);
                ...
                TcPremierSalesForm.ResetMasterFormFilter = true;
            }
            else
            {
                DataLoaded = false;
        ...
        {
            SetFilterData(SalaryTable.HasEmployeeNumberDuplicates(), TcEnum.GetTextForEnum<TePremierSalesSalaryFilter>(TePremierSalesSalaryFilter.Employee_Number_Duplicates));
            SetFilterData(SalaryTable.HasNICDuplicates(), TcEnum.GetTextForEnum<TePremierSalesSalaryFilter>(TePremierSalesSalaryFilter.NIC_Duplicates));

            filterComboBox.Text = TcEnum.GetTextForEnum<TePremierSalesSalaryFilter>(TePremierSalesSalaryFilter.All);
        }
        ...
        private void dataGridView_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                TcPremierSalesSalaryRow row = source.Current as TcPremierSalesSalaryRow;

                if (row != null)
                {
                    TePremierSalesSalaryFilter filter = TcEnum.GetEnumForText<TePremierSalesSalaryFilter>(TePremierSalesSalaryFilter.All, filterComboBox.Text);
                    switch (filter)
                    {
                        case TePremierSalesSalaryFilter.All:
                            duplicatesSource.DataSource = SalaryTable.GetDuplicates(row.EmployeeNumber, row.NIC);
                            break;

                        case TePremierSalesSalaryFilter.Employee_Number_Duplicates:
                            duplicatesSource.DataSource = SalaryTable.GetEmployeeNumberDuplicates(row.EmployeeNumber);
                            break;

                        case TePremierSalesSalaryFilter.NIC_Duplicates:
        ...
        private void Search()
        {
            ...
            TcSearchHelper<TcPremierSalesSalaryRow> searchHelper = new TcSearchHelper<TcPremierSalesSalaryRow>();
        ...
        private void SetStatus()
        {
            ...
            TcBindingList<TcPremierSalesSalaryRow> list = source.DataSource as TcBindingList<TcPremierSalesSalaryRow>;

            decimal total = 0;

            foreach (TcPremierSalesSalaryRow row in list)
            {
