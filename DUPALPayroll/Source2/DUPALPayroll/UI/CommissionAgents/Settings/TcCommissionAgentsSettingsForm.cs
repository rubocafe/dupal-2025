...
namespace DUPALPayroll.UI.CommissionAgents.Settings
{
    public partial class TcCommissionAgentsSettingsForm : Form
    {
        ...
        public string CommissionsFilePath { get; set; }
        public string CommissionsHeldFilePath { get; set; }

        public TcYearMonth WorkingYearMonth { get; private set; }

        public TcCommissionAgentsSettingsForm(TcCommissionAgentsForm master)
        {
            InitializeComponent();
        ...
        private bool IsValid()
        {
            ...
            MasterFilePath = Path.Combine(RootDirectoryPath, string.Format("CommissionAgentsMaster_{0}.csv", suffix));
            ...
            CommissionsFilePath = Path.Combine(RootDirectoryPath, string.Format("Commissions_{0}.csv", suffix));
            if (!File.Exists(CommissionsFilePath))
            {
                TcMessageBox.ShowWarning(string.Format("Commissions file [{0}] not found. Please add Commissions file", CommissionsFilePath));
                return false;
            }

            CommissionsHeldFilePath = Path.Combine(RootDirectoryPath, string.Format("CommissionsHeld_{0}.csv", suffix));
            if (!File.Exists(CommissionsHeldFilePath))
            {
                TcMessageBox.ShowWarning(string.Format("Commissions Held file [{0}] not found. Please add Commissions Held data file", CommissionsHeldFilePath));
                return false;
            }
