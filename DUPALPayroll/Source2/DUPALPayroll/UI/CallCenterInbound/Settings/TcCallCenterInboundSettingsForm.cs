...
namespace DUPALPayroll.UI.CallCenterInbound.Settings
{
    public partial class TcCallCenterInboundSettingsForm : Form
    {
        ...
        public TcCallCenterInboundSettingsForm(TcCallCenterInboundForm master)
        {
            InitializeComponent();
        ...
        private bool IsValid()
        {
            ...
            MasterFilePath = Path.Combine(RootDirectoryPath, string.Format("CallCenterInboundMaster_{0}.csv", suffix));
            ...
            SalaryFilePath = Path.Combine(RootDirectoryPath, string.Format("CallCenterInboundSalary_{0}.csv", suffix));
            if (!File.Exists(SalaryFilePath))
            {
                TcMessageBox.ShowWarning(string.Format("{0} Salary file [{1}] not found. Please add salary file", master.Identifier, SalaryFilePath));
                return false;
            }
