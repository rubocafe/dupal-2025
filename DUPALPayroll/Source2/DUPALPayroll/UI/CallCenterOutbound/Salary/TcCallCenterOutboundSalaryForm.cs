...
{
    public partial class TcCallCenterOutboundSalaryForm : Form
    {
        private TcCallCenterOutboundForm master;
        private string filePath = string.Empty;

        private TcBindingList<TcCallCenterOutboundSalaryRow> all = new TcBindingList<TcCallCenterOutboundSalaryRow>();
        ...
        public TcCallCenterOutboundSalaryTable SalaryTable { get; private set; }

        public TcCallCenterOutboundSalaryForm(TcCallCenterOutboundForm master, string filePath)
        {
            InitializeComponent();

            this.filePath           = filePath;
            this.master             = master;
            SalaryTable             = new TcCallCenterOutboundSalaryTable();
            DataLoaded              = false;

            dataGridView.AutoGenerateColumns            = false;
            duplicatesDataGridView.AutoGenerateColumns  = false;

            source.DataSource           = new TcBindingList<TcCallCenterOutboundSalaryRow>();
            duplicatesSource.DataSource = new TcBindingList<TcCallCenterOutboundSalaryRow>();
            ...
            filterComboBox.Items.Add(TcEnum.GetTextForEnum<TeCallCenterOutboundSalaryFilter>(TeCallCenterOutboundSalaryFilter.All));
        ...
        public void ReloadData()
        {
            if (File.Exists(filePath))
            {
                ...
                TcCallCenterOutboundSalaryLoader loader = new TcCallCenterOutboundSalaryLoader(master.SettingsForm.WorkingYearMonth);
                ...
                TcCallCenterOutboundForm.ResetMasterFormFilter = true;
            }
            else
            {
                DataLoaded = false;
        ...
        {
            SetFilterData(SalaryTable.HasEmployeeNumberDuplicates(), TcEnum.GetTextForEnum<TeCallCenterOutboundSalaryFilter>(TeCallCenterOutboundSalaryFilter.Employee_Number_Duplicates));
            SetFilterData(SalaryTable.HasNICDuplicates(), TcEnum.GetTextForEnum<TeCallCenterOutboundSalaryFilter>(TeCallCenterOutboundSalaryFilter.NIC_Duplicates));

            filterComboBox.Text = TcEnum.GetTextForEnum<TeCallCenterOutboundSalaryFilter>(TeCallCenterOutboundSalaryFilter.All);
        }
        ...
        private void dataGridView_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                TcCallCenterOutboundSalaryRow row = source.Current as TcCallCenterOutboundSalaryRow;

                if (row != null)
                {
                    TeCallCenterOutboundSalaryFilter filter = TcEnum.GetEnumForText<TeCallCenterOutboundSalaryFilter>(TeCallCenterOutboundSalaryFilter.All, filterComboBox.Text);
                    switch (filter)
                    {
                        case TeCallCenterOutboundSalaryFilter.All:
                            duplicatesSource.DataSource = SalaryTable.GetDuplicates(row.EmployeeNumber, row.NIC);
                            break;

                        case TeCallCenterOutboundSalaryFilter.Employee_Number_Duplicates:
                            duplicatesSource.DataSource = SalaryTable.GetEmployeeNumberDuplicates(row.EmployeeNumber);
                            break;

                        case TeCallCenterOutboundSalaryFilter.NIC_Duplicates:
        ...
        private void Search()
        {
            ...
            TcSearchHelper<TcCallCenterOutboundSalaryRow> searchHelper = new TcSearchHelper<TcCallCenterOutboundSalaryRow>();
        ...
        private void SetStatus()
        {
            ...
            TcBindingList<TcCallCenterOutboundSalaryRow> list = source.DataSource as TcBindingList<TcCallCenterOutboundSalaryRow>;

            decimal total = 0;

            foreach (TcCallCenterOutboundSalaryRow row in list)
            {
