...
{
    public partial class TcSupervisorsAndBackOfficeSalaryForm : Form
    {
        private TcSupervisorsAndBackOfficeForm master;
        private string filePath = string.Empty;

        private TcBindingList<TcSupervisorsAndBackOfficeSalaryRow> all = new TcBindingList<TcSupervisorsAndBackOfficeSalaryRow>();
        ...
        public TcSupervisorsAndBackOfficeSalaryTable SalaryTable { get; private set; }

        public TcSupervisorsAndBackOfficeSalaryForm(TcSupervisorsAndBackOfficeForm master, string filePath)
        {
            InitializeComponent();

            this.filePath           = filePath;
            this.master             = master;
            SalaryTable             = new TcSupervisorsAndBackOfficeSalaryTable();
            DataLoaded              = false;

            dataGridView.AutoGenerateColumns            = false;
            duplicatesDataGridView.AutoGenerateColumns  = false;

            source.DataSource           = new TcBindingList<TcSupervisorsAndBackOfficeSalaryRow>();
            duplicatesSource.DataSource = new TcBindingList<TcSupervisorsAndBackOfficeSalaryRow>();
            ...
            filterComboBox.Items.Add(TcEnum.GetTextForEnum<TeSupervisorsAndBackOfficeSalaryFilter>(TeSupervisorsAndBackOfficeSalaryFilter.All));
        ...
        private void ApplyTheme()
        {
            ...
            TcTheme.FormatGridCurrencyDisplayColumn(otNormalColumn);
            TcTheme.FormatGridCurrencyDisplayColumn(otDoubleColumn);
            TcTheme.FormatGridCurrencyDisplayColumn(netSalaryColumn);
            TcTheme.FormatGridCurrencyDisplayColumn(pbiColumn);
            TcTheme.FormatGridCurrencyDisplayColumn(workTravelAllowanceColumn);
            ...
            TcTheme.FormatGridCurrencyDisplayColumn(dOTNormalColumn);
            TcTheme.FormatGridCurrencyDisplayColumn(dOTDoubleColumn);
            TcTheme.FormatGridCurrencyDisplayColumn(dNetSalaryColumn);
            TcTheme.FormatGridCurrencyDisplayColumn(dPbiColumn);
            TcTheme.FormatGridCurrencyDisplayColumn(dWorkTravelAllowanceColumn);
        ...
        public void ReloadData()
        {
            if (File.Exists(filePath))
            {
                ...
                TcSupervisorsAndBackOfficeSalaryLoader loader = new TcSupervisorsAndBackOfficeSalaryLoader(master.SettingsForm.WorkingYearMonth);
                ...
                TcSupervisorsAndBackOfficeForm.ResetMasterFormFilter = true;
            }
            else
            {
                DataLoaded = false;
        ...
        {
            SetFilterData(SalaryTable.HasEmployeeNumberDuplicates(), TcEnum.GetTextForEnum<TeSupervisorsAndBackOfficeSalaryFilter>(TeSupervisorsAndBackOfficeSalaryFilter.Employee_Number_Duplicates));
            SetFilterData(SalaryTable.HasNICDuplicates(), TcEnum.GetTextForEnum<TeSupervisorsAndBackOfficeSalaryFilter>(TeSupervisorsAndBackOfficeSalaryFilter.NIC_Duplicates));

            filterComboBox.Text = TcEnum.GetTextForEnum<TeSupervisorsAndBackOfficeSalaryFilter>(TeSupervisorsAndBackOfficeSalaryFilter.All);
        }
        ...
        private void dataGridView_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                TcSupervisorsAndBackOfficeSalaryRow row = source.Current as TcSupervisorsAndBackOfficeSalaryRow;

                if (row != null)
                {
                    TeSupervisorsAndBackOfficeSalaryFilter filter = TcEnum.GetEnumForText<TeSupervisorsAndBackOfficeSalaryFilter>(TeSupervisorsAndBackOfficeSalaryFilter.All, filterComboBox.Text);
                    switch (filter)
                    {
                        case TeSupervisorsAndBackOfficeSalaryFilter.All:
                            duplicatesSource.DataSource = SalaryTable.GetDuplicates(row.EmployeeNumber, row.NIC);
                            break;

                        case TeSupervisorsAndBackOfficeSalaryFilter.Employee_Number_Duplicates:
                            duplicatesSource.DataSource = SalaryTable.GetEmployeeNumberDuplicates(row.EmployeeNumber);
                            break;

                        case TeSupervisorsAndBackOfficeSalaryFilter.NIC_Duplicates:
        ...
        private void Search()
        {
            ...
            TcSearchHelper<TcSupervisorsAndBackOfficeSalaryRow> searchHelper = new TcSearchHelper<TcSupervisorsAndBackOfficeSalaryRow>();
        ...
        private void SetStatus()
        {
            ...
            TcBindingList<TcSupervisorsAndBackOfficeSalaryRow> list = source.DataSource as TcBindingList<TcSupervisorsAndBackOfficeSalaryRow>;

            decimal total = 0;

            foreach (TcSupervisorsAndBackOfficeSalaryRow row in list)
            {
