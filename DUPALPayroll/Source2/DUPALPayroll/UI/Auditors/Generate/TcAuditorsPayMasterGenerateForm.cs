...
{
    public partial class TcAuditorsPayMasterGenerateForm : Form
    {
        private TcAuditorsForm master;
        private BindingSource source = new BindingSource();

        public TcAuditorsPayMasterGenerateForm(TcAuditorsForm master)
        {
            InitializeComponent();
            ...
            try
            {
                if (AnalyzeCompleted())
                {
                    if (InputValid())
                    {
                        ...
                        TcOutputFilesGenerator<TcAuditorsAnalyzedRow> generator
                            = new TcOutputFilesGenerator<TcAuditorsAnalyzedRow>
                                (master.SettingsForm.RootDirectoryPath, originData, master.SettingsForm.WorkingYearMonth);
        ...
        private void SetSummaryLabel()
        {
            ...
            TcPayMasterRowsValidator<TcAuditorsAnalyzedRow> validator =
                new TcPayMasterRowsValidator<TcAuditorsAnalyzedRow>(originData, master.AnalyzeForm.PaymasterDataList);
            validator.Validate();
        ...
        {
            TcSalaryConditionsChecker<TcAuditorsAnalyzedRow> checker = new TcSalaryConditionsChecker<TcAuditorsAnalyzedRow>(
        ...
        {
            referenceTextBox.Text = string.Format("AUD SAL {0} '{1}",
        ...
        private bool GenerateSalarySlips()
        {
            ...
            TcAuditorsSalarySlipsCreator creator = new TcAuditorsSalarySlipsCreator(master.SettingsForm.WorkingYearMonth);
        ...
        private void TcAuditorsPayMasterGenerateForm_Load(object sender, EventArgs e)
        {
            try
            {
